...
function Calculate() {
  ...
  const makeSecondApiCall = (accessToken) => {
    ...
    const url = 'http://localhost:8080/https://ob.sandbox.natwest.com/open-banking/v3.1/pisp/bnpl-payment-consents';
    const headers = {
...
const makeFifthApiCall = (accessToken, consentId) => {
  console.log('totalCost: '+totalCost);
  const url = 'http://localhost:8080/https://ob.sandbox.natwest.com/open-banking/v3.1/pisp/bnpl-payments';
  const idempotencyKey = uuidv4();
  const headers = {
    'Authorization': `Bearer ${accessToken}`,
    'Content-Type': 'application/json',
